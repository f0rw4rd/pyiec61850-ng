...
class FileInfo:
    ...
    def last_modified_datetime(self) -> Optional[datetime]:
        ...
        if self.last_modified:
            try:
                return datetime.fromtimestamp(self.last_modified / 1000.0, tz=timezone.utc)
...
class FileClient:
        ...
        try:
            ...
            if isinstance(result, tuple):
                ...
                if error != iec61850.IED_ERROR_OK:
                    raise FileError(f"Failed to list directory '{directory}': error {error}")
            else:
                file_list = result
    ...
    def download_file(
        ...
        try:
            ...
            if not hasattr(iec61850, "MmsConnection_fileOpen"):
                raise FileError("MMS file open API not available in SWIG bindings")
            ...
            if isinstance(result, tuple):
                ...
                if isinstance(error_val, int) and error_val != 0:
                    raise FileError(f"Failed to open file '{filename}': error {error_val}")
            else:
                frsmId = result
    ...
    def delete_file(self, filename: str) -> bool:
        ...
        try:
            ...
            if error != iec61850.IED_ERROR_OK:
                raise FileError(f"Failed to delete '{filename}': error {error}")
    ...
    def rename_file(self, old_name: str, new_name: str) -> bool:
        ...
        try:
            ...
            if error != iec61850.IED_ERROR_OK:
                raise FileError(f"Failed to rename '{old_name}' to '{new_name}': error {error}")
